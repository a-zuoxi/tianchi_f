...
from fast_adv.attacks.attacks import DDN
...
parser.add_argument('--save-folder', '--sf', default='weights/cifar10_random/', help='folder to save state dicts')
parser.add_argument('--save-freq', '--sfr', default=3, type=int, help='save frequency')
...
model_file='/media/unknown/Data/PLP/fast_adv/defenses/weights/best/2Atten_PLP_cifar10_ep_9_val_acc0.8780.pth'
...
for epoch in range(args.epochs):
    ...
    for i, (images, labels) in enumerate(tqdm.tqdm(train_loader, ncols=80)):
        ...
        if args.adv is not None and epoch >= args.adv:
            ...
            logits_adv = model(adv_noise.detach())
        ...
        accs.append((logits_adv.argmax(1) == labels).float().mean().item())
    ...
    with torch.no_grad():
        for i, (images, labels) in enumerate(tqdm.tqdm(val_loader, ncols=80)):
            ...
            logits = model(noisy_inputs.detach())
            '''
            if args.shape is not None and epoch >= args.shape:
                image_shape = torch.zeros_like(images)
                for i in range(args.batch_size):
                ...
                logits=model.forward(image_shape.detach())
                '''
    ...
    if val_accs.avg >= best_acc:  # args.adv is None and
        ...
        files2remove = glob.glob(os.path.join(save_path, '2random_*'))
        for _i in files2remove:
            os.remove(_i)
        strsave = "2random_smooth_cifar10_ep_%d_val_acc%.4f.pth" % (epoch, best_acc)
