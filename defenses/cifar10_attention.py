...
parser.add_argument('--save-folder', '--sf', default='weights/ALP+shape_0.25_mixed_Attention/', help='folder to save state dicts')
...
for epoch in range(args.epochs):
    ...
    for i, (images, labels) in enumerate(tqdm.tqdm(train_loader, ncols=80)):
        ...
        if args.shape is not None and epoch >= args.shape:
            ...
            logits,feature_mix=model.forward_attention(images.detach(),image_shape.detach())
            loss_attention=F.cross_entropy(logits, labels)
            #attention
            #loss=loss_adv+loss_attention
            #attention+plp
            loss = loss_adv + loss_attention+ 0.2*F.mse_loss(logits_adv,logits_clean)+0.1*F.mse_loss(feature_adv,feature_clean)
    ...
    if val_accs.avg >= best_acc:  # args.adv is None and
        ...
        files2remove = glob.glob(os.path.join(save_path, '0.25Mixed+ALP_*'))
        for _i in files2remove:
            os.remove(_i)
        strsave = "0.25Mixed+ALP_cifar10_ep_%d_val_acc%.4f.pth" % (epoch, best_acc)
