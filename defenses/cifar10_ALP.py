...
from fast_adv.defenses.shape_precess import shape
import torch
...
parser.add_argument('--save-folder', '--sf', default='weights/AT+ALP/', help='folder to save state dicts')
...
for epoch in range(args.epochs):
    ...
    for i, (images, labels) in enumerate(tqdm.tqdm(train_loader, ncols=80)):
        ...
        if args.adv is not None and epoch >= args.adv:
            ...
            #print(logits_adv.size(),feature_adv.size())
    ...
    if val_accs.avg >= best_acc:  # args.adv is None and
        ...
        files2remove = glob.glob(os.path.join(save_path, 'ALP_*'))
        for _i in files2remove:
            os.remove(_i)
        strsave = "ALP_ATcifar10_ep_%d_val_acc%.4f.pth" % (epoch, best_acc)
