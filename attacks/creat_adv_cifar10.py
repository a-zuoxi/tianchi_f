...
from tianchi2021.attacks.M_DI2_FGSM import M_DI2_FGSM_Attacker
from models.resnet.FaceBook_resnet import resnet101_denoise
...
parser.add_argument('--max_norm', default=20, type=float, help='Linf limit')
...
#             pretrained_model
#         )
# model1 = NormalizedModel(model=model, mean=image_mean, std=image_std)
...
model = load_model("resnet152_ddn_jpeg").to(DEVICE).eval()
...
for batch_data in tqdm.tqdm(train_loader):
    ...
    if args.m_di2_fgsm:
        ...
        out_path = './tianchi2021_adv/m_di2_fgsm_newloss_resnet152_jpeg_l220_step100/'
        ...
        for t in range(args.batch_size):
            ...
            if not os.path.exists(out_path):
                os.makedirs(out_path)
            out = out_path + name
            # adv_img = scipy.misc.imresize(adv_img)
